...
function Payments() {
  ...
  useEffect(() => {
    axios
      .get(`managerBills/${manager.m_id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('userToken')}` } })
      .then((res) => {
                      ...
                      <Tr key={i + 1}>
                        ...
                        <Td>
                          <HStack>
                            ...
                            filled
                            <Button variant={'filled'} size={['xs', 'sm']} bgColor={'red.500'} onClick={() => paymentDeleteHandler(p.b_id)}>
                              Del
                            </Button>
                          </HStack>
                        </Td>
