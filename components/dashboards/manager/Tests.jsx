...
function Tests() {
  ...
  const testAddHandler = () => {
    const id = { m_id: manager.m_id };
    Object.assign(changeForm, id);
    ...
    axios
      ...
      .catch((err) => {
        ...
        toast({
          title: error.t_location || error.t_name || error.t_category || error.t_price || error.reporting_time,
          status: 'error',
          duration: '3000',
          isClosable: true,
        });
  ...
  useEffect(() => {
    axios
      .get(`managerTests/${manager.m_id}`, { headers: { Authorization: `Bearer ${localStorage.getItem('userToken')}` } })
      .then((res) => {
